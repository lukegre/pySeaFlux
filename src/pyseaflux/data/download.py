...
def get_data(data_config_entry: dict):
    ...
    logger.info(
        f"Combing {len(ds_groups)} datasets into one group with time freq of {output_freq}"
    )
...
def get_data_from_url_list(url_list: tuple[str], entry: dict):
    ...
    flist = download_netcdfs(
        url_list, entry.get("fsspec_options"), downloader=entry.get("downloader", None)
    )
    ...
    if entry.get("output_options", {}).get("delete_raw_files"):
        if not isinstance(flist[0], str):
            raise ValueError(
                "You can only delete raw files when you're not working with zip files"
            )
        logger.warning(
...
def make_output_name(
    ds: xr.Dataset, item_config: dict, template: str = "{name}-{var}-{time}.nc"
):
    kwargs = {
